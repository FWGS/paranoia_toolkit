...
extern vec3_t	render_origin; // buz
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if (m_pCurrentEntity->curstate.renderfx == 70)
		{
			...
			if (!gl.Paranoia_hacks)
				return 0; // dont draw skybox entites, DepthRange will be incorrect
	...
	if (flags & STUDIO_RENDER)
	{
		...
		else
		{
			...
			if (m_pCurrentEntity->curstate.renderfx == 60)
			{
				...
				gLightLevel = (float)lighting.shadelight / 192;
				return 0;
			}
		...
		if ((m_pCurrentEntity->curstate.renderfx == 50) ||
			(m_pCurrentEntity->curstate.renderfx == 49)) // buz test
		{
			...
			{
			default:
			case 50: pweaponmodel = IEngineStudio.Mod_ForName("models/mask.mdl", 1); break;
			case 49: pweaponmodel = IEngineStudio.Mod_ForName("models/zombie_eyes.mdl", 1); break;
			}
			...
			if (cv_renderer->value && cv_nsr->value) GL_SetupLighting(); // buz
...
void CStudioModelRenderer::StudioRenderModel( void )
{
	...
	{
	//	gEngfuncs.pTriAPI->RenderMode( kRenderTransTexture ); // buz tests
...
void CStudioModelRenderer::StudioRenderFinal_Hardware( void )
{
	...
	if (cv_renderer->value && cv_nsr->value) GL_SetupRenderer( rendermode ); // buz
	...
	else
	{
		for (i=0 ; i < m_pStudioHeader->numbodyparts ; i++)
		{
			if (cv_renderer->value && cv_nsr->value) // buz
				GL_SetupModel( i );
			else
				IEngineStudio.StudioSetupModel( i, (void **)&m_pBodyPart, (void **)&m_pSubModel );
	...
	// buz: why they dinnt put this in hardware renderer?
	if (m_pCvarDrawEntities->value == 5)
	{
		IEngineStudio.StudioDrawAbsBBox( );		
	}

	if (cv_renderer->value && cv_nsr->value) GL_RestoreRenderer(); // buz
