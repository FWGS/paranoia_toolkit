...
{
	lighting_ext	light;
	DynamicLight*	dlights[MAX_MODEL_DYNLIGHTS];
};*/
...
public:
	...
	int StudioDrawHeadShield(); // buz test
	int StudioDrawCustomEntity(cl_entity_t *ent, int amb = -1, int shade = -1); // buz
...
//	void Lighting (float *lv, int bone, int flags, vec3_t normal);
	void Lighting (vec3_t &lv, int bone, int flags, vec3_t normal);
	void Chrome (int *pchrome, int bone, vec3_t normal);
};



void EXT_LightPoint(vec3_t pos, lighting_ext *lightinfo, int invlight); // buz
